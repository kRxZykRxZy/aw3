...
Blockly.Variables.validateScalarVarOrListName_ = function (
    ...
    if (
        workspace.getVariable(name, type) ||
        additionalVars.indexOf(name) >= 0
    ) {
        // error
...
Blockly.Variables.getVariable = function (workspace, id, opt_name, opt_type) {
    ...
    if (id) {
        ...
        if (opt_type == undefined) {
            throw new Error(
                "Tried to look up a variable by name without a type"
            );
        }
...
Blockly.Variables.createVariable_ = function (
    ...
    if (potentialVariableMap) {
        var variable = potentialVariableMap.createVariable(
            opt_name,
            opt_type,
            id
        );
