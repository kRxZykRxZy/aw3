...
Blockly.VerticalFlyout.prototype.setMetrics_ = function (xyRatio) {
    ...
    this.clipRect_.setAttribute(
        "height",
        Math.max(0, metrics.viewHeight) + "px"
    );
...
Blockly.VerticalFlyout.prototype.clearOldBlocks_ = function () {
    ...
    for (var checkboxId in this.checkboxes_) {
        if (
            !Object.prototype.hasOwnProperty.call(this.checkboxes_, checkboxId)
        ) {
            continue;
        }
...
Blockly.VerticalFlyout.prototype.layout_ = function (contents, gaps) {
    ...
    for (var i = 0, item; (item = contents[i]); i++) {
        if (item.type == "block") {
            ...
            if (this.RTL) {
                button.moveTo(
                    flyoutWidth - this.MARGIN - button.width,
                    cursorY
                );
            } else {
