...
const getAllMd5exts = () => {
    ...
    if (!fs.existsSync(costumesManifest)) {
        throw new Error(
            "costumes.json does not exist -- did you forget a step?"
        );
    }
    if (!fs.existsSync(backdropManifest)) {
        throw new Error(
            "backdrops.json does not exist -- did you forget a step?"
        );
    }
    if (!fs.existsSync(spriteManifest)) {
        throw new Error(
            "sprites.json does not exist -- did you forget a step?"
        );
    }
