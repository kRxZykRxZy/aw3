...
const getSourceDateEpoch = () => {
    ...
    if (gitProcess.error) {
        if (gitProcess.error === "ENOENT") {
            console.warn(
                "Could not get source date epoch: git is not installed"
            );
...
const flipFuses = async context => {
    ...
    newFuses[
        electronFuses.FuseV1Options.EnableEmbeddedAsarIntegrityValidation
    ] = false;
...
const buildWindowsLegacy = () =>
    build({
        ...
        extraConfig: {
            nsis: {
                artifactName:
                    "${productName}-Legacy-Setup-${version}-${arch}.${ext}",
            },
...
const buildMacLegacy1015 = () =>
    build({
        ...
        extraConfig: {
            mac: {
                artifactName:
                    "${productName}-Legacy-10.15-Setup-${version}.${ext}",
            },
        ...
        extraConfig: {
            ...
            artifactBuildCompleted: async artifact =>
                new Promise((resolve, reject) => {
                    console.log(
                        `Running strip-nondeterminism on ${artifact.file}`
                    );
...
const run = async () => {
    ...
    if (built === 0) {
        console.log(
            "Need to specify platforms; see release-automation/README.md"
        );
        process.exit(1);
    }
};
