...
{#if typeof window !== 'undefined' && (!hiddenAlerts.includes(id) || !closable)}
	<div
		class="p-2 font-bold flex items-center justify-between transition-all"
	...
	>
		<div class="flex min-h-[40px] flex-grow items-center justify-center">
			...
			{#if button}
				<a
					href={button.uri}
					class="ml-2 px-4 py-2 text-sm font-bold cursor-pointer rounded-full no-underline"
		...
		{#if closable}
			<button
				class="px-2 text-lg ml-auto cursor-pointer"
