...
export const POST: RequestHandler = async ({ request, cookies }) => {
	try {
        ...
        const sb3 = encodeBase64(new Uint8Array(await request.arrayBuffer()));
        const now = new Date().toISOString();
        const projectMeta = { id: Math.floor(Math.random()*1e9), title:'Untitled Project', description:'', instructions:'', visibility:'visible', public:true, comments_allowed:true, is_published:true, author:{id:user.id, username:user.username, scratchteam:false, history:{joined:user.joined.toISOString()}, profile:{id:null, images:{'90x90':'','60x60':'','55x55':'','50x50':'','32x32':''}}}, image:'', images:{}, history:{created:now, modified:now, shared:now}, stats:{views:0,loves:0,favorites:0,remixes:0}, remix:{parent:null, root:null}, project_token:crypto.randomUUID() };
        const projectId = crypto.randomUUID();
        await db.insert(table.project).values({ id:projectId, title:projectMeta.title, instructions:projectMeta.instructions, notes:projectMeta.description, creator:user.id, ghost:false, projectJson:sb3, projectMeta });
